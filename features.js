...
router.post('/order', async (req, res) => {
    ...
    DatabaseService.identifyToken(token).then(async (resp) => {
        if (resp) {
            switch (type2) {
                case "limit": // 지정가
                    switch (type1) {
            ...
            res.json({ message: "Matching token not found in the database :(" });
        }

    }).catch((err) => {
        res.json({ message: err.message });
    });; // checks for matching token in the db, if not error message sent to client
...
router.get('/fetch', async (req, res) => {
    ...
    DatabaseService.identifyToken(token).then((resp) => {
        if (resp) {
